...
/* Modif 8 Decembre 1997 : on inverse les periodes >100s au premier run. 
 ...
 if (compt==9) compt=4; /* This line is here becuase want to inverse only T < 100 s */
 if (compt==4) 
    {    
     /* basterdisation of this code to make it invert only 50 and 70 seconds */
     npertot=2;   
     if (f40==0 || f40==1)
          {pertot[0]=50;  /* pertot[0]=30; */
           pertot[1]=70;} 
     if ((f40==0 || f40==1) && (idata==-2))
          {per[0]=70; 
           nper=1;   }
     else if ((f40==0 || f40==1) && (idata>=-1)) 
          {per[0]=50;  /*per[0]=30; */
           per[1]=70; 
           nper=2;}
    }
...
if((day>365) || (day<0)) 
 {printf ("%s\n Peculiar Earthquake not compatible with sortdata1bis");
  exit(1);}
  /*
    printf("\n ");
