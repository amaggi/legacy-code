...
*/
  ...
  for (ipath=0;ipath<npaths;ipath++) {
    ...
    fgets(ligne,400,indat);
    sscanf(ligne,"%f %f %f %f",&ela,&elo,&sla,&slo);
    ...
    slon[ipath]=(double)slo;
    coslat[ipath]=(double)sla;
    if(slon[ipath]<0.) slon[ipath]=slon[ipath]+360.;
    if((slon[ipath]<0.) || (slon[ipath]>360.)) {
      printf ("\n Pb with lon path: slon= %f path %d \n ",slon[ipath],ipath);
      exit(1);
    }
