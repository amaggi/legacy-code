...
c---------------------------------------------------------
c 28/01/04
c VERSION adaptee a regiostack6 
...
c     print *,'entrer latitude et longitude initiales'
c     print *,'Pour l\'Australie, 20. 90. '
c     print *,'Pour la corne de l\'Afrique, 55. 0. '
c     read(*,*) y0,x0
      y0=0.
      x0=-180.
...
c    *file='/home1/eric/these/models/des.1066a.rovs')
      ...
      open(12,status='old',
     *file='/home/eric//models/des.prem.rovs.int')
      endif
       read(12,*)
      read(12,*)
      read(12,*) npr
...
c valeur test pour compariason avec Eric Beucler (C 60 s)
cc       if (tt.eq.60.)cref=3.971
...
c        lon=(x+j)*xpas 
         ...
         write(iout,'(f8.3,1x,f8.3,2x,f8.3,2x,i5,2x,i5)')
     *lat,lon,cp(ii,jj),jj,ii
         write(ierout,'(f8.3,1x,f8.3,2x,f8.3)')lat,lon,er(ii,jj) 
...
c     write(iaout2,'(2i5,2f8.3)')lat,lon,(phiao(i,j)+180),alpha(i,j)
             write(iaout,'(4(f8.3,1x))')lat,lon,phiao(i,jj),alpha(ii,jj)
             write(iaout2,'(4(f8.3,1x))')lat,lon,(phiao(ii,jj)+180)
     *,alpha(i,jj)
...
c       lon=(x+j)*xpas
       ...
       if(icoup.eq.1.and.latcoup(jc).eq.lat)then
          ...
          write(iCerout(jc),1200)profs(iprof),lon,er(i,jj)
        endif
140   continue
