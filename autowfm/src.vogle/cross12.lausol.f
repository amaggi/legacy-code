c     Version de cross12.lr.auto.F faite pour tourner avec la bibliotheque
c     graphique Vogle
c     Version pour LINUX ( 04/06/2002)
     ...
     *          xis(nfr),f(nfr),yrm(nfr),yim(nfr),n1(nfdim),nb(nfdim),
     *          ppar(nfdim),pper(nfdim),xsave(ndim)
     ...
     *          idatam(nmode,nfdim,invd),qual(nlob),qualo(nmode,nfdim)
      ...
      character sort*80,entr*80,entref*80,bcd(4)*80,titre*80,tit1*80
      real*8 pi2
...
c--------------------------------------------------------------
      ...
      icross=0
      isegment=1
c     write(0,*) 'Option dessin est necessaire pour :'
c     write(0,*) '    - calcul de enveloppe'
...
c
      ...
      icoul=8
c     START BYPASS
c     call initdes(xg,xd,y0,yt,iview,input,idim,icoul,isegment)
c	call CreateRetainSeg(1)
c     END BYPASS
      yt=yt-10.
      endif
c
c  Preparation une inversion des intercorrelogrammes
c
98    ider=0
      ipic=0
      write(*,*)
     *  'voulez-vous piquer les valeurs de l intercorrelogramme?'
      if(nonoui(0).eq.1)then
      ...
      ipic=1
      write(*,*)
     * 'voulez-vous creer un fichier pour calcul des deriv partielles?'
         if(nonoui(0).eq.1)then
         ider=1
...
c
c Lecture des trois fichiers entree
c
1     write(*,*)'fichier contenant le sismogramme reel?'
      read(*,'(a)') entr
      write(*,'(a)')entr
      open(is,status='old',err=1,file=entr)
      ...
      close(is)
c
22    write(*,*) 'fichier contenant le signal de syntetique?'
      read(*,'(a)')entr
      write(*,'(a)')entr
...
C     endif                                                             k l
c     call SetLinestyle(DOTTED)                                         k l
      ienv0=ienv
      ndata=ndatam(k,l)
      eyy=ey(k,l)
...
33    call flush(6)
c     START BYPASS
c        call CloseRetainSeg(isegment)
c     END BYPASS
      if(ider.eq.1) then
      close(ioo)
      close(io)
...
c
      write(*,'("filtrage anti-repliement du sismogramme reel:")')
...
8     xi(i)=0.
      ...
      per=1./dnu/(npp-1)
      write(*,'("la periode de coupure du filtre est ",f10.4)')per
      if2=1./per/dnn+1.5
      do 17 i=if2+1,nnp
      xr(i)=0.
17    xi(i)=0.
...
c     START BYPASS
c     call CloseRetainSeg(isegment)
c     isegment=isegment+1
c     call CreateRetainSeg(isegment)
c     END BYPASS
        nsave=n
        idsave=idata(1)

c     Bypassing all the plotting by outputting the data to be
c     plotted to files(dependent on type of data)
c     The original plotting routines will be left in place
c     and will work as before.
c     iout : the unit number for output of data
c     
c     START BYPASS
c     find out if file is open - and if not open it and rewind
c     do not close file - it will be closed by program exit
      write(*,*) 'INQUIRE for plot.out'
      inquire(file='plot.out',opened=fileopen,number=iout)
      if (.not. fileopen) then
        iout=27
        open(file='plot.out',unit=iout)
        rewind(iout)
      endif
c     END BYPASS
...
c     	call MoveAbs2(xg,ybase)
c     	call LineAbs2(xd,ybase)
      	xc=xg+(n-1)*ex/2.
c     	call MoveAbs2(xc,yc-dh/2)
c     	call LineAbs2(xc,yc-dt)
c     	Dessin de l'echelle-temps (60 s)
      	ycc=yc+dh/4
      	top=dh/20
      	xmn=60*ex/pas+xg
c     	call MoveAbs2(xg+top,ycc+top)
c     	call LineAbs2(xg+top,ycc)
c     	call LineAbs2(xmn+top,ycc)
c     	call LineAbs2(xmn+top,ycc+top)
      endif
c     START BYPASS
      do 141 i=1,n
141     write(iout,*) xw(i), yw(i)
c    END BYPASS

c     Dessin du signal
c     call MoveAbs2(xw(1),yw(1))
c     call PolyLineAbs2(xw,yw,n)
c
...
c           call moveabs2(xw(icourrant),yw(icourrant))
c           call lineabs2(xw(icourrant),yw(icourrant)+dh/5)
c           call moveabs2(xw(icourrant),ybase)
c           call lineabs2(xw(icourrant),ybase-dh/5)
c           jmax(ii)=icourrant
...
c	dessin des tirets de pickage
      do 7 i=1,ndata
c     call MoveAbs2(xw(idata(i)),yw(idata(i)))
c     call LineAbs2(xw(idata(i)),yw(idata(i))-dh/3)
   7  continue
c
...
c      START BYPASS
c	call CloseRetainSeg(isegment)
c      	call DelRetainSegment(isegment-1)
c      	call DelRetainSegment(isegment)
c      START BYPASS
      	isegment=isegment+1
c      	call CreateRetainSeg(isegment)
      	npic=0
...
c     un pas de 0.5 km/s.
      ...
      dimension pper(*),qualit(nmode,nfdim)
      dimension ey(nmode,nfdim)
      character texte*20, entr*(*), titre*80, forma*80

c     call SetCharPrecision(CHARACTER)
c     call SetFont(STICK)
c     call SetCharSize((xd-xg)*.015,dh*.15)

c     call MoveAbs2(xg,yt+8.5)
c     call Text(titre)

c     call SetCharSize((xd-xg)*.01,dh*.10)

c     call MoveAbs2(xd-0.5*dh,y0+2.6*dh)
c     call Text('resid.')
c     Dessin du nom du fichier synthetique pour l'iteration en cours
c     call MoveAbs2(xd-0.5*dh,y0+1.6*dh)
c     call Text(entr)
      write(titre,'(a,f5.2)') 'x',amult
c     call MoveAbs2(xd-0.5*dh,y0+1.3*dh)
c     call Text(titre)
c     call MoveAbs2(xd-0.5*dh,y0+0.6*dh)
c     call Text('data')
...
c     call MoveAbs2(xg+(xd-xg)*.005,y0+0.10*dh)
c     call Text(texte)

c     call MoveAbs2(xg,y0)
...
c     call LineAbs2(xx,y0)
c     call LineAbs2(xx,y)
c     call LineAbs2(xx,y0)
    1 continue

c     call LineAbs2(xd,y0)

      if((xd-xx).lt.(xd-xg)*0.075) xx=xd-(xd-xg)*0.080
      write(texte,'(f3.1,a)') kmin/2., ' km/s'
c     call MoveAbs2(xx+(xd-xg)*.005,y0+0.10*dh)
c     call Text(texte)
...
c      call LineAbs2(xmn+top,ycc)
c      call LineAbs2(xmn+top,ycc+top)
c     call MoveAbs2(xg+(xd-xg)*.02,y0+0.83*dh)
c     call Text('60 s')
...
c     Trace des periodes des cross-corr.
      ...
      do 2 i=1,nper
c     call SetCharSize((xd-xg)*.01,dh*.10)
c     call MoveAbs2(xcal+(i-1)*xdec,yt+2.)
...
c  	iq=0 ==> qualites identiques
   ...
   22 continue
c     call SetCharSize((xd-xg)*.006,dh*.06)
      if(iq.eq.0.and.abs(qualit(1,i)).lt.1.e-4) goto 299
c       On n'ecrit ce titre que si on a picke des donnees sur les
c       cross-correlogrammes.
	ncar=3
	do 123 j=1,km
	if(qualit(j,i).ne.0.) nc=log10(int(100*qualit(j,i))+1.e-6)+1
...
c     call MoveAbs2(xg+0.2*(xd-xg)/nper+(i-1)*xdec,yt-0.5)
c     call Text(titre)
  299 continue
      if(iey.eq.0) then
c       On n'ecrit ce titre que si l'echelle est la meme pour toute
c       la colonne.
c     call MoveAbs2(xg+0.4*(xd-xg)/nper+(i-1)*xdec,y0+(yt-y0)*0.485)
...
c     Trace des numeros de mode a gauche des cross-corr.
      ...
      ydec=(yt-y0)/(2*km)
c     call SetCharSize((xd-xg)*.008,dh*.08)
c     call MoveAbs2(xg,ycal+(km-0.5)*ydec)
c     call Text('mode')
c     call SetCharSize((xd-xg)*.012,dh*.12)
      do 3 i=1,km
c     call MoveAbs2(xg,ycal+(i-1)*ydec)
...
c     call SetCharSize((xd-xg)*.008,dh*.08)
      do 4 i=1,nper-1
      do 4 j=1,km
c     call MoveAbs2(xcal+(i-1)*xdec,ycal+(j-1)*ydec)
c     call Text('data')
c     call MoveAbs2(xcal+(i-1)*xdec,ycal+(j-1)*ydec+yyd)
c     call Text('synt')
    4 continue
