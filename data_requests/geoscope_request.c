...
int main(int argc, char *argv[]){
  ...
  if(argc != 4){
    printf("Usage geoscope_request events.txt stations.dat userprofile\n");
    return;
  }
  ...
  switch(selection_type){
    case(MT5):
      geoscope_mt5_request(station_list, n_stations, event_list, n_events, argv[3]);
      break;
    case(STATION):
      geoscope_station_request(station_list, n_stations, event_list, n_events, argv[3]);
      break;
    case(RANGE):
      geoscope_range_request(station_list, n_stations, event_list, n_events, argv[3]);
      break;
    default:
