...
!BL
	...
	REAL(DDP), DIMENSION(:), INTENT(IN) :: src
	REAL(DDP), DIMENSION(:), INTENT(OUT) :: dest
	INTEGER(I4B), INTENT(OUT) :: n_copied, n_not_copied
	n_copied=min(size(src),size(dest))
	n_not_copied=size(src)-n_copied
	dest(1:n_copied)=src(1:n_copied)
...
!BL
	...
	REAL(DDP), INTENT(IN) :: first,increment
	INTEGER(I4B), INTENT(IN) :: n
	REAL(DDP), DIMENSION(n) :: arth_d
	INTEGER(I4B) :: k,k2
	REAL(DDP) :: temp
...
!BL
	...
	REAL(DDP), INTENT(IN) :: first,factor
	INTEGER(I4B), INTENT(IN) :: n
	REAL(DDP), DIMENSION(n) :: geop_d
	INTEGER(I4B) :: k,k2
	REAL(DDP) :: temp
...
!BL
	...
	REAL(DDP), DIMENSION(:), INTENT(IN) :: first,factor
	INTEGER(I4B), INTENT(IN) :: n
	REAL(DDP), DIMENSION(size(first),n) :: geop_dv
	INTEGER(I4B) :: k,k2
	REAL(DDP), DIMENSION(size(first)) :: temp
...
!BL
	...
	REAL(DDP), DIMENSION(:), INTENT(IN) :: coeffs
	REAL(DDP) :: poly_dd
	REAL(DDP) :: pow
	REAL(DDP), DIMENSION(:), ALLOCATABLE :: vec
	INTEGER(I4B) :: i,n,nn
	n=size(coeffs)
	if (n <= 0) then
		poly_dd=0.0_ddp
...
!BL
	...
	REAL(DDP), DIMENSION(:), INTENT(IN) :: coeffs,x
	REAL(DDP), DIMENSION(size(x)) :: poly_ddv
	INTEGER(I4B) :: i,n,m
	m=size(coeffs)
	n=size(x)
	if (m <= 0) then
...
!BL
	...
	REAL(DDP), DIMENSION(:), INTENT(IN) :: coeffs,x
	LOGICAL(LGT), DIMENSION(:), INTENT(IN) :: mask
	REAL(DDP), DIMENSION(size(x)) :: poly_msk_ddv
	poly_msk_ddv=unpack(poly_ddv(pack(x,mask),coeffs),mask,0.0_ddp)
...
!BL
	...
	REAL(DDP), DIMENSION(:), INTENT(IN) :: a,b
	REAL(DDP), DIMENSION(size(a),size(b)) :: outerprod_d
...
!BL
	...
	REAL(DDP), DIMENSION(:), INTENT(IN) :: a,b
	REAL(DDP), DIMENSION(size(a),size(b)) :: outerdiff_d
...
!BL
	...
	REAL(DDP), DIMENSION(:), INTENT(OUT) :: dest
	REAL(DDP), DIMENSION(:), INTENT(IN) :: source
	INTEGER(I4B), DIMENSION(:), INTENT(IN) :: dest_index
	INTEGER(I4B) :: m,n,j,i
...
!BL
	...
	REAL(DDP), DIMENSION(:,:), INTENT(IN) :: mat
	REAL(DDP), DIMENSION(size(mat,1)) :: get_diag_dv
	INTEGER(I4B) :: j
