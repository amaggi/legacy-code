c---programme de filtrage  variable
c--  lecture des donnees en format SAC
c---------------------------------------------------------------------
      character cod*8,chan*8,infile*50,outfile*50,outfile1*50,filevit*50
      character outfileyref*50, outfileyf*50
...
c---------------------------------------------------------------
      ...
      luin=1
      luout=10
c-----This program is on an infinite loop that starts here.  To break
c-----the loop put 'fin' as the input sac filename
 1    continue
      infile='      '  
	  outfile='     '
	  npo=0
...
c---------------------------------------------------------------
      write(*,*)'Name of the SAC file to be filtered: '
      read(*,'(a50)')infile
      write(*,'(a50)')infile
...
c ntot= nb de mode   ntrain=nbre de train a traiter
      write(*,*) 'Number of modes and number of wave-trains:'
      read(*,*)ntot,ntrain
      write(*,*)'ntot,ntrain',ntot,ntrain
      
      write(*,*) 'Give tdeb, tfin, ipi, ips:'
      read(*,*) tdeb,tfin,ipi,ips
      write(*,*) tdeb,tfin,ipi,ips
      
	  write(6,*) 'Name of group velocity file: '
      read(*,'(a50)')filevit
      write(*,'(a50)')filevit

c----------------------------------------------------------------
c     Read group velocity file
c---------------------------------------------------------------      
	  open(19,file=filevit,status='old')
	  do  k=1,ntot
 	 	read(19,*) nutrain(k),nmode(k),nbper(k)
		print*,nutrain(k),nmode(k),nbper(k)
      	read (19,*) (tt1(k,i),i=1,nbper(k))
      	write(*,*) (tt1(k,i),i=1,nbper(k))
		read (19,*) (uu1(k,i),i=1,nbper(k))
		write(*,*) (uu1(k,i),i=1,nbper(k))
	  enddo
	  close (19)
	
	  ! loop over modes 0 to 4 (??)
	  do 109 kkk=0,4
		nk=0
		n=0
		
		! initialise periods and group velocities
		do i=1,55
		  ta1(i)=0.
		  ua1(i)=0.
		enddo
		...
		! create filename for output SAC file
		write(outfile,'(a1,i1)') 'n',kkk
		outfile=outfile(1:2)//outfile1(1:lnblnk(outfile1))
		write(outfileyref,'(a1,i1)') 'n',kkk
		outfileyref =
     *      outfileyref(1:2)//outfile1(1:lnblnk(outfile1))//".ref"
		write(outfileyf,'(a1,i1)') 'n',kkk
		outfileyf=outfileyf(1:2)//outfile1(1:lnblnk(outfile1))//".f"
	
		
		
		! initialise station/event/seismogram parameters
        cod='  '
        chan='  '
        alat=0.
        alon=0.
...
!        mode=0
        ...
        ! read sac file
        maxpts=33000
        call rsac1(infile,y,npoint,bsac,pas,maxpts,ierr)
        if (ierr.ne.0) then
          write(6,*)'pb ouverture fichier de lecture'
     *              ,infile,ierr
          goto 99
        endif
        ! read header parameters from current sac file
        call getfhv('stla',alat,ierr)
        call getfhv('stlo',alon,ierr)
        call getfhv('stel',sdep,ierr)
        call getfhv('evla',elate,ierr)
        call getfhv('evlo',elonge,ierr)
        call getfhv('evdp',edep,ierr)
        call getkhv('kstnm',cod,ierr)
        call getkhv('kcmpnm',chan,ierr)
...
!        endif
        ...
        print*,'debut enregistrement',bsac
        print*,'nb points',npoint,' dt',pas
...
!      s=(lj-ida)*24.*3600.+(ihd-ih0)*3600.+(imd-im0)*60.+sd-s0
        ! s= trecord_start - tevent, which for my sac files is 0.0
        s=bsac 
        if (s) 40,40,41
   40   nst = s/pas-0.5
        go to 43
...
!		goto 99
!	endif
        write(*,*) 'DEBUG : ', outfileyref, outfileyf, outfile
        call wsac1(outfileyref,yref,npoint,0.0,pas,ierr)
...
!        call xdrf_wrahrecord0(luout,ida,ih0,im0,s0,m,ierr,yf)
        call wsac1(outfileyf,yf,npoint,0.0,pas,ierr)
      	if(ierr.ne.0)then
 	      write(*,*)'erreur ecriture yf',ierr
 	      goto 99
	    endif
!        call xdrf_wrahrecord0(luout,ida,ih0,im0,s0,m,ierr,y)
        call wsac1(outfile,y,npoint,0.0,pas,ierr)
      	if(ierr.ne.0)then
