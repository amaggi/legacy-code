c
c $Id: mkhomog.f,v 1.1.1.1 2002/07/12 11:15:20 maggi Exp $
c
c $Log: mkhomog.f,v $
...
c
c
       subroutine mkhomog(idfl,c1,s1,c2,s2)
...
c  and richards derivation (1981).
      ...
      real*8 sddep,w,ccc,gmmm,py1,py2,py3,ddt
...
c  open files containing excitations
      read(idfl) nsrce,npts,ddt,jcom,nbran
...
c  find nearest source depth in file to event depth
	...
	ikr=iki-1
c  read in love excitations
        read(idfl,err=555) w,ccc,gmmm,b1,gb1
        c=sngl(ccc)
        gamm=sngl(gmmm)
         do 35 k=1,nsrce
         read(idfl) py1,py2
