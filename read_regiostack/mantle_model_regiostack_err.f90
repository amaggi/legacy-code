...
!  double precision, parameter ::    RMOHO = 6346600.d0
  ...
  integer, parameter :: NLAT = 45, NLON = 90, NLAY = 18
  double precision, parameter :: XPAS = 4.0d0
  double precision, parameter :: YPAS = 4.0d0
  ...
  logical, parameter :: OUTPUT_ABSOLUTE = .false.
...
!-----------------
  ...
  double precision dummy(1:NLAT,1:NLON,1:NLAY)
...
! read vsfin file
  open(unit=10,file='vsfin-vect',status='old')
  do k=1,NLAY
    read(10,*) depth
    ...
    do j=1,NLAT
      read(10,*) (dummy(j,i,k),i=1,NLON)  
    enddo
    ! read the absolute errors
    do j=1,NLAT
      read(10,*) (grid_vs(j,i,k),i=1,NLON)  
      do i=1,NLON
        ! Turn absolute velocites into relative velocites
          grid_vs(j,i,k) = (grid_vs(j,i,k) / dummy(j,i,k)) 
        ! If we want to output absolute velocities, then apply 
        ! the perturbation to PREM
        if (OUTPUT_ABSOLUTE) then
           !grid_vs(j,i,k) = (grid_vs(j,i,k)) * quick_prem_vsv(grid_r(k))
           grid_vs(j,i,k) = (grid_vs(j,i,k)) * bg_vs
        endif
      enddo
    enddo
  enddo
...
! factor to convert perturbations in shear speed to perturbations in density
  ...
  call bracket(grid_phi,NLON,phi,iphi)
