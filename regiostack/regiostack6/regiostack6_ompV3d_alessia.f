*  Pgm for "continuous regionalization" using massive datsets
...
c      integer OMP_GET_THREAD_NUM
      ...
      real*8 err,tol,re
      real fcoagcgt,fcoacgt

      integer nxny,nxm,nym,nyp,nypxy,nynxm
...
cJJL  du modele initial (et non par rapport a la moyenne des donnees)
        ...
        enddo
        varini=varini/NTRA
        write(*,*)'variance-donnees p.r. au modele initial:       ',
     &             varini
...
c--x0 et xe ont ete passe en lenteur, normalement redvar est en lenteur 
        ...
        write(*,*)'variance-donnees p.r. a la moyenne des mesures:',
     &             varmoy
        write(*,*)'variance-donnees p.r. au modele final:         ',
     &             varres
...
c comme ca.
        ...
        print*,'qui  (res. quadr. normalise des donnees en lenteur)=',
     &          qui
c       calcul de quim
        quim=0.
...
c     REAL,DIMENSION(:),allocatable::ulo
      real ulo(NSTEP)
      ...
      integer i,ki1,nx,nstep

      real sd,pc1,pc2,pc3,pc4,d1,d2,d3,d4
      real fi,tei,um,cp,ct,ph,tet,sn,cs


      integer icp1,icp,ict1,ict,kki
...
c
c     ALLOCATE(ulo(NSTEP),stat=ierr)
c     if(ierr/=0) then
c       print *,"Erreur a L\'allocation de ulo"
c       stop 4
c     endif
      v=0.
c     print *,'pbdirc'
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      real function cosdel(tetam1,phim1,tetam2,phim2)
      real c1,c2,s1,s2,tetam1,phim1,tetam2,phim2
      cosdel=0.
      c1=cos(tetam1)
      c2=cos(tetam2)
      s1=sin(tetam1)
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      real function sindel(tetam1,phim1,tetam2,phim2)
      real a,co,un,tetam1,phim1,tetam2,phim2
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     *,delt,ij,ki,kd1,store,ipos,dul_tmp,an1_tmp,an2_tmp,teta,phi)
      ...
      real dcorlim,dcor2,dcoran2,sigma2,sigan2,fcoagcgt,fcoacgt
      real fpxys2,fnys2,pimpxy
      real xinf,yinf,pxy
      real xo,yo,nxo,nyo,pxyo
      real rad
      integer nx,ny,nxny,nxm,nym,nyp,nypxy,nynxm

      integer k,jj,jcurrent,inb,kk,j2,ip2,it2,ij,ki
      real fcoa2,fcoa1,fcoa,fco,tx2,py,tx
...
c    *cov,mask,ij,ki,teta,phi,dsi,kdi,nray,store,ipos)
     *cov,mask,ij,ki,teta,phi,dsi,kdi,nray,jj,jcurrent,store,ipos)
      ...
      integer k,jj,jcurrent,inb,ik,ir,kk,j2,ip2,it2,ki,ij,kki
      real dsik,dski,fcoa2,fcoa1,fcoa,fco,tx2,py,tx
...
c       call asolve(sa,ija,n,b,z,0)
        call asolve(sa,n,b,z)
        bnrm=snrm(n,z,itol)
...
c       call asolve(sa,ija,n,r,z,0)
        call asolve(sa,n,r,z)
        znrm=snrm(n,z,itol)
      else
...
c       call asolve(sa,ija,n,rr,zz,1)
        call asolve(sa,n,rr,zz)
        bknum=0.d0
        do j=1,n
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C  (C) Copr. 1986-92 Numerical Recipes Software Y"1p3-1.
c     SUBROUTINE asolve(sa,ija,n,b,x,itrnsp)
      SUBROUTINE asolve(sa,n,b,x)
c     REAL*8,DIMENSION(:),allocatable::sa
c     INTEGER*4,DIMENSION(:),allocatable::ija
c     INTEGER n,itrnsp,i,ija(*),NTRAJ
c     INTEGER n,itrnsp,i,ija(*)
      INTEGER n,i
      real*8 x(*),b(*),sa(*)
c     PARAMETER (NTRAJ=6000)
c     COMMON /mat/ sa,ija
