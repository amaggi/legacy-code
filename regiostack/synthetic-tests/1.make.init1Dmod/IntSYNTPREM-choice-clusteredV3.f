...
c alessia: modified to take V3 formatted intomodes files
      ...
      real plume_adb, plume_VsTgrad(NPROFMAX,NTMAX) 
      real plume_profs(NPROFMAX), plume_T(NTMAX)
      integer n_adb, n_plume_profs

     
      real mag, pert_size, profmin,profmax, gridstep, intstep
      integer anis_type, psign, synt_type, nanis
...
c     ask about anisotropy:
      write(*,*) 'Plot anisotropy every how-many model steps?'
      read(*,*) nanis
...
c           find the latitude and longitude of this point
            lat= 90-((i-1)*gridstep+gridstep/2)
            lon=(j-1)*gridstep+gridstep/2
...
c   Ecritures des cartes a differentes profondeurs
        crefpass=cref(jp)
        call writedata(c,a1,a2,filein,jp,zone,crefpass,gridstep,nanis)

c--------COUPES DANS LE MODELE INITIAL-----------------------------

c------------------------------------------------------------------
c      great circle cross-sections
c------------------------------------------------------------------
        do 140 jc=1,nbcoup
          alat1=lat1(jc)
          alon1=lon1(jc)
...
140     continue
c------------------------------------------------------------------
c      latitude cross-sections      
c------------------------------------------------------------------
        do 17 ii=1,nbcouplat
          ilatc=int((90-latc(ii))/gridstep)+1
          do 17 jlon=1,nlon
            lon=(jlon-1)*gridstep
...
1002    format(a40)
        ...
        if(itra.eq.802) write(*,*)lats(itra),lons(itra)
...
c       write(isor,1003) (vout(ii)/100.,ii=1,ncouch)
