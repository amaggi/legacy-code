...
c   Ecritures des cartes a differentes profondeurs
      crefpass=cref(jp)
      call writedata(c,a1,a2,filein,jp,zone,crefpass)
...
125    lentsom(jp)=0
      ...
      do 130 itra=1,nbtra
       stabid=sta(itra)
      nomtrajet=stabid(:lnblnk(stabid))
      elat=late(itra)
      elon=lone(itra)
      stabid=sta(itra)
      sta2=stabid(1:4)
      ...
      if (itra.lt.10) then
      write(isor,1004) sta2,".z0",itra,nomtrajet(:lnblnk(nomtrajet))
      elseif (itra.lt.100) then
      write(isor,1005) sta2,".z",itra,nomtrajet(:lnblnk(nomtrajet))
      elseif (itra.lt.1000) then
      write(isor,1006) sta2,".z",itra,nomtrajet(:lnblnk(nomtrajet))
      elseif (itra.lt.10000) then
      write(isor,1007) sta2,".z",itra,nomtrajet(:lnblnk(nomtrajet))
      endif
...
c     remove output to des files for clustered paths
c     cannot do full synthetic test from here anyway, because of
c     the statistical nature of the clustering
...
c       write(isor,1003) (err(itra,ii),ii=1,25)
        write(isor,1003) (vsout(2)/100.,ii=1,ncouch)
