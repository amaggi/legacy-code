...
c	We cannot use the des. files directly for the clustered case
c	anyway, so do not output them.
c       toto='des.S'//nomtrajet(:lnblnk(nomtrajet))//'....LHZ'
c       write(*,'(a)')toto 
c       open(9,file=toto,status='new') 
c       write(9,'(a)') 'Synthetic within PREM + dvs'
c       write(9,*)' 1'
c       write(9,*)ncouch-2,nvar

        do 120 ii=1,ncouch
c         write(9,*)prof(ii),vout(ii), 0.0050
...
c       write(*,*) 'Done ', toto
        ...
        write(isor,1003) (vout(ii)/100.,ii=1,ncouch)
