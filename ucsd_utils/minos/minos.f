...
c*** makes up table of frequencies ***
  ...
  100 format(/,'integration precision =',g12.4,'  root precision =',
     ...
     +   8x,'q',13x,'raylquo',/)
      ...
      print *,'enter lmin,lmax,wmin,wmax,nbran'
      read(*,*) lmin,lmax,wmin,wmax,nbran
      wmin=max(wmin,0.1)
      wmin=wmin*cmhz
      wmax=wmax*cmhz
...
c*** determine mode count ***
      we(1)=wt(1)
!      print *, 'DEBUG calling detqn'
      call detqn(we(1),ke(1),de(1),0)
!      print *, 'DEBUG, called detqn'
      imax=2*nmx
...
c      print 900,wt(1),wt(kkk),nmode,nbran
  900 format('count between ',g14.6,' and',g14.6,' =',i8,' : max =',i4)
      ...
      nmode=min(nmode,nbran)
      imax=2*nmode
c*** fill up table using bisection ***
      indx=2
      ichk=ke(1)+1
   35 if(ke(indx).eq.ichk) then
        indx=indx+2
...
c*** find roots by spline interpolation ***
     ...
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
      common/mtab/we(430),de(430),ke(430),wtry(250),bm(250),um(250)
      dimension x(20),det(20),qx(3,20),wrk(60),wt(*)
      data tol/1.d-9/,itmax/15/
      ichar="stsc"
      knev=1
      nmo=nev/2
      do 100 i=1,nmo
...
c      print 900,b,fb
      ...
      print 200,nord,ichar(jcom:jcom),l,b,wmhz,tcom,gcom,qmod,wdiff
      write(iout,200) nord,ichar(jcom:jcom),l,b,wmhz,tcom,gcom,qmod,wdiff
  200 format(i5,a2,i5,6g16.7)
      call modout(b,qmod,gcom,ioeig)
      if(nord.eq.0) wmin=b
      if(bm(nind).le.0.d0) goto 70
c      blin=b+cg*wn
c      bp2=5.d0*bm(nind)-4.d0*b+2.d0*(um(nind)+2.d0*cg*wn)
c      bdiff=dmax1(dabs(blin-bp2),eps1*bp2)
c      bup=bp2+bdiff
c      bdwn=bp2-bdiff
c      wtry(nind)=bdwn
c      goto 71
  ...
  100 continue
     ...
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
      ...
      common/buff/nn,ll,ww,qq,gc,buf(2100)
      dimension abuf(2105)
      equivalence (nn,abuf)
      nn=nord
      ll=l
   ...
   10 continue
      ...
      common/mkstuff/r(mk),fmu(mk),flam(mk),qshear(mk),qkappa(mk),
...
!      print *,'DEBUG starting detqn with wdim, knt, det, ifeif', wdim,
!     +         knt, det, ifeif
      iback=0
      w=wdim/wn
      wsq=w*w
...
!      print *, 'DEBUG : just before strange goto statement'
      goto (2,3,1,3),jcom
    1 if(wdim.le.wgrav) kg=1
!      print *, 'DEBUG : just after 1'
      nvefm=2+kg*3
      nvesm=5+kg*9
!      print *, 'DEBUG calling sdepth'
      call sdepth(wdim,ls)
      if(ls.le.2) then
...
!        print *, 'DEBUG propagate inner core'
...
!        print *, 'DEBUG propagate outer core'
        is=max(ls,nicp1)
...
c*** radial modes ***
    2 ls=2
!      print *, 'DEBUG : just after 2'
!      print *, 'DEBUG calling rps with ls ass = ', ls, ass
!      print *, 'DEBUG g(ls) = ', g(ls)
      call rps(ls,ass)
!      print *, 'DEBUG calling rprop'
      call rprop(ls,n,ass)
      ...
      do i=2,n
        ...
        enddo
      enddo
!      print *, 'DEBUG: returning from subroutine'
      return
c*** toroidal modes ***
    3 if(jcom.eq.2) then
!      print *, 'DEBUG : just after 3'
        nb=nocp1
        n2=nsl
        ass(1)=1.d0
        ass(2)=0.d0
...
!        print *, 'DEBUG within grav subroutine del, rn2, trn, c1-5',
!     +            del, rn2, trn, c1, c2, c3, c4, c5
        g(i)=(g(im1)*rn2+4.d0*del*(c1+del*(c2+del*(c3+del*(c4
     +   +del*(c5+del*qro(3,im1)/6.d0))))))/(r(i)*r(i))
!        print *, 'DEBUG i, g(i)', i, g(i)
      enddo
      return
      end
...
!      print *, 'DEBUG : sig, 1/sig = ', sig, 1/sig
!      print *, 'DEBUG i,r, rho, wsq, g = ',i, r(i),rho(i),wsq,g(i)
      zsq=r(i)*r(i)*rho(i)*(wsq+4.d0*g(i)/r(i))/sig
!      print *, 'DEBUG : calling bfs with ', 1, zsq,eps,fp
      call bfs(1,zsq,eps,fp)
      a(1)=r(i)
      a(2)=sig*fp+2.d0*fla
!      print *, 'DEBUG : returning from rps'
      return
      end
...
c  described in w. lentz's article in applied optics, vol.15, #3, 1976
      ...
      save
      real*8 numer,nu
!      print *, 'DEBUG: bfs inputs l xsq eps fr',l,xsq,eps,fp
c*** positive argument uses continued fraction
      if(xsq.gt.0.d0) then
!      print *, 'DEBUG : continued fraction'
        x=sqrt(xsq)
        lp1=l+1
        rx=2.0d0/x
        nu=lp1-0.5d0
...
c  series solution
!      print *, 'DEBUG : series solution, eps=',eps
        f=1.d0
        fp=l
        a=1.d0
        b=l+l+1.d0
